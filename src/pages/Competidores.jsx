...
const marketSignals = [
  {
    ...
    signal: 'Gestão é estratégica para delivery. iFood quer o "all in one". Valida o mercado.',
    tamyResponse: 'Janela de 12-18 meses antes de bundle possível. Prioridade: stickiness via open finance e literacia financeira.',
    icon: '🍕',
  },
  {
    date: 'Out/2025',
...
const brCompetitors = [
  {
    ...
    aiLevel: 'none',
    weakness: 'Zero AI. Onboarding requer treinamento presencial. UX datada. iFood subsidia mas ainda nao fez bundle.',
    threat: 'high',
  },
  {
    name: 'Simpliza',
    ...
    aiLevel: 'none',
    weakness: 'Ancora freemium dificulta upsell. Sem financeiro profundo. Sem proatividade.',
    threat: 'low',
  },
  {
    name: 'Sischef',
    ...
    aiLevel: 'none',
    weakness: 'Posicionamento generico. Suporte e o unico diferencial. Sem AI.',
    threat: 'low',
  },
  {
    name: 'Consumer PDV',
    ...
    aiLevel: 'none',
    weakness: '15 anos de legado tecnologico. Nenhum sinal de AI roadmap. Volume alto, inercia alta.',
    threat: 'low',
  },
  {
    name: 'F360',
    ...
    focus: 'Gestao financeira multi-canal para franquias',
    aiLevel: 'early',
    weakness: 'ICP e franquia 15+ unidades. Nao serve Carlos nem Marcia. Ticket minimo alto. Comecou AI em 2025 (boletins WhatsApp).',
    threat: 'medium',
  },
]
...
const dimensions = [
  {
    ...
    others: 'Todos os sistemas BR mostram dados quando voce pergunta. Reativos.',
  ...
  },
  {
    ...
    others: 'DRE generico, fluxo de caixa basico, relatorios estaticos.',
    tamy: 'CMV por categoria, margem por prato, DRE por canal (delivery vs. salao), deteccao de desperdicio, analise de sazonalidade, precificacao com impostos. Cada skill treinada nas especificidades do food service BR.',
    color: 'border-blue-500',
  },
  {
    icon: '🏦',
    dimension: 'Open Finance automatico (Pluggy)',
    others: 'F360 faz isso para franquias. Para independentes: zero cobertura.',
    tamy: 'Conecta banco do Carlos via Pluggy. Reconcilia iFood/Rappi com extrato bancario automaticamente. Detecta taxas indevidas e discrepancias. Feature com stickiness imediata — quem conectou nao desconecta.',
    color: 'border-emerald-500',
  },
  {
    icon: '💬',
    dimension: 'Chat-first, nao menu-first',
    others: 'Interface tradicional: menu → submenu → relatorio. Precisa saber navegar.',
    tamy: 'WhatsApp + Web + Telegram. Carlos fala "quanto ganhei essa semana?" as 00h30 pelo WhatsApp e tem a resposta em 30s com contexto e recomendacao.',
    color: 'border-pink-500',
  },
  {
    icon: '📚',
    dimension: 'IA que ensina, nao so mostra numeros',
    others: 'Mostram numeros. Carlos ainda nao sabe o que fazer.',
    tamy: 'Tamy explica o que o numero significa, por que esta errado e o que fazer. "Seu CMV esta em 42% — o ideal para bar e 28-35%. Se renegociar a carne com fornecedor X, voce economiza R$1.200/mes." Aumenta a literacia financeira.',
    color: 'border-amber-500',
  },
  {
    icon: '📱',
    dimension: 'Multi-canal nativo BR',
    others: 'App ou web. Nenhum tem WhatsApp + Telegram + Web integrado.',
    tamy: '169M+ usuarios de WhatsApp no Brasil. Zero friccao de adocao. Carlos ja usa WhatsApp — Tamy vive onde ele ja esta.',
  ...
  return (
    <div>
      <SectionHeader
        ...
        desc="Por que a Tamy ocupa uma posicao unica no mercado — e por que nenhum concorrente brasileiro tem AI-native + financeiro profundo + canais BR simultaneamente."
      ...
      <div className="reveal mb-8">
        ...
        <p className="text-gray-500 text-sm mb-4">
          O mercado brasileiro de gestao para food service — nenhum e AI-native, nenhum tem financeiro proativo
        </p>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          <StatCard value="0" label="Competidores BR com AI nativa" color="text-red-500" />
          <StatCard value="0" label="Com open finance para independentes" color="text-red-500" />
          <StatCard value="0" label="Com chat financeiro proativo" color="text-red-500" />
          <StatCard value="79+" label="Skills financeiras da Tamy" color="text-purple-600" />
        </div>
        ...
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-900">
          <strong>Risco #1 — iFood Bundle:</strong> iFood tem Saipos (PDV) + OPDV (gestao) + 3S Checkout (pagamentos) + Anota AI (WhatsApp ordering, 70k restaurantes). Se fizer bundle gratis, cria "bom o suficiente". <strong>Defesa:</strong> iFood ainda esta integrando 3 aquisicoes simultaneas. Tamy tem 12-18 meses para criar stickiness via open finance + literacia financeira.
        </div>
      </div>
      ...
      <div className="reveal mb-8">
        ...
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'linear-gradient(135deg, #1e1b4b, #312e81)' }}>
          ...
          <div className="space-y-2">
            {[
              { who: 'Saipos, Simpliza, Sischef, Consumer', has: 'Eixo 3 (Brasil)', miss: 'Sem AI, sem financeiro profundo' },
              { who: 'F360', has: 'Eixo 2 (financeiro) + Eixo 3 parcial', miss: 'ICP e franquia, nao independente. AI inicial.' },
              { who: 'Toast IQ, Restoke, MarginEdge', has: 'Eixo 1 (AI)', miss: 'Nao operam no Brasil. Sem Pluggy, sem WhatsApp.' },
              { who: 'Tamy', has: 'Eixo 1 + 2 + 3', miss: 'Nenhum gap. Unica na interseccao completa.' },
            ].map(({ who, has, miss }, i) => (
              <div key={who} className={`rounded-xl p-3 ${i === 3 ? 'bg-purple-500/30 border border-purple-400/50' : 'bg-white/10'}`}>
                <div className="flex items-start gap-2">
                  {i === 3 && <span className="text-yellow-300 font-bold shrink-0">★</span>}
                  <div>
                    <span className={`text-xs font-bold ${i === 3 ? 'text-yellow-200' : 'text-white'}`}>{who}</span>
                    <span className="text-purple-200 text-xs"> — {has}</span>
                    {i < 3 && <p className="text-red-300 text-xs mt-0.5">{miss}</p>}
                    {i === 3 && <p className="text-green-300 text-xs mt-0.5 font-semibold">{miss}</p>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="reveal rounded-2xl border border-purple-200 p-5"
        ...
        <div className="text-center">
          ...
          <h3 className="font-bold text-purple-900 text-base mb-2">
            "Saipos te diz que voce vendeu R$12k."
          </h3>
          <p className="text-purple-700 text-sm leading-relaxed max-w-lg mx-auto mb-3">
            <strong>Tamy te diz que voce lucrou R$1.400</strong> depois de iFood (27%), desperdicio (15%) e custo fixo — e que o prato de file esta te custando R$180 a cada venda. E sugere o que fazer.
          </p>
          <p className="text-purple-600 text-xs font-semibold">
            Essa e a diferenca entre um sistema de gestao e um copiloto financeiro AI-native.
          </p>
        </div>
      </div>
    </div>
